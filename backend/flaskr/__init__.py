...
def create_app(test_config=None):
    ...
    def search_questions():
        ...
        body = request.get_json()
        search = body.get('searchContent', None)
        try:
            questions = Question.query \
              .order_by(Question.id) \
              .filter(Question.question.like('%{}%'.format(search)))
    ...
    def retrieve_quizes():
        try:
            questions = None
            body = request.get_json()
            quiz_categories = body.get('quiz_categories', None)
            previous_ids = body.get('previous_question', None)
